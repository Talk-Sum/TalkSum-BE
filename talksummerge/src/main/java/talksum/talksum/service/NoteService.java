...
public class NoteService {
    ...
    private static final int PAGE_NOTE_COUNT=5; //한 페이지에 존재하는 노트 수


    /* 노트 목록 불러오기_ 휴지통으로 이동하지 않은 노트들만 보여야 한다! */
    @Transactional
    public List<NoteDto> getNoteList(Member member, Integer pageNum) {
        Pageable pageable = PageRequest.of(pageNum - 1, PAGE_NOTE_COUNT, Sort.by(Sort.Direction.ASC, "createdDate"));
...
/*
    ...
    public void emptyTrash(Member loginMember) {
        ...
        noteRepository.deleteNotesByAuthorAndIsTrash(loginMember, true);
    }
